...
public partial class MainViewModel : ObservableObject, IDisposable
{
    ...
    private readonly IFileTypeDetectorService  _fileTypeDetector;   // FIX C-1: now injected
    ...
    // ── Event-handler subscription lists ──────────────────────────────────
    ...
    /// <summary>
    /// Phase 4: Raised when the user taps "+ Custom tag" in the prompt builder.
    /// Code-behind shows a form dialog and calls AddCustomXmlTagAsync with the result.
    /// </summary>
    ...
    [NotifyPropertyChangedFor(nameof(IsLocalPathMode))]
    [NotifyPropertyChangedFor(nameof(IsGitHubMode))]
    ...
    // ── Phase 3: Local path mode ───────────────────────────────────────────

    /// <summary>
    /// True when the repo URL field contains a valid local filesystem path.
    /// When true, the Branch and Token sections are hidden and a guidance
    /// banner is shown explaining the 3-step local-path workflow.
    /// </summary>
    public bool IsLocalPathMode =>
        !string.IsNullOrWhiteSpace(RepoUrl) && IsLocalPath(RepoUrl.Trim());

    /// <summary>True when the repo URL points to a GitHub remote.</summary>
    public bool IsGitHubMode =>
        !string.IsNullOrWhiteSpace(RepoUrl) && !IsLocalPath(RepoUrl.Trim());
    ...
    public string FetchedFilesCountLabel =>
        $"FILE TREE ({FetchedFiles.Count} files)";

    public bool HasFetchedFiles => FetchedFiles.Count > 0;

    // ── Phase 5: Flat tree for BindableLayout display ─────────────────────

    private readonly List<FileTreeNode>                   _treeRoots  = [];
    private readonly ObservableCollection<FileTreeNode>   _flatFileTree = [];

    /// <summary>
    /// Flattened, visibility-filtered tree. Only nodes whose ancestors are
    /// all expanded appear here. Rebuilt on every expand/collapse event.
    /// BindableLayout renders this directly.
    /// </summary>
    public ObservableCollection<FileTreeNode> FlatFileTree => _flatFileTree;

    public bool HasFileTree => _flatFileTree.Count > 0;

    private void RebuildFlatTree()
    {
        var visible = FileTreeBuilder.Flatten(_treeRoots);
        _flatFileTree.Clear();
        foreach (var n in visible) _flatFileTree.Add(n);
    ...
    public string CompactSummaryText
    {
        get
        {
            ...
            var br   = IsLocalPathMode ? "local" : (string.IsNullOrWhiteSpace(Branch) ? "—" : Branch);
    ...
    partial void OnSelectedSavedRepoChanged(SavedRepo? value)
    {
        ...
        // FIX F-8: Clear the token field when selecting a repo that has no saved token.
        // Previously the old token stayed visible, misleading the user about auth state.
        if (value.HasToken)
        {
    ...
    /// <summary>Phase 4 — XML tag buttons for the prompt builder palette.</summary>
    public ObservableCollection<XmlTagButton> XmlTagButtons { get; } = [];
    ...
    public MainViewModel(
        ...
        IFileTypeDetectorService  fileTypeDetector,
    ...
    public async Task InitializeAsync()
    {
        ...
        await RefreshXmlTagButtonsAsync();
    }
    ...
    private async Task RefreshXmlTagButtonsAsync()
    {
        try
        {
            XmlTagButtons.Clear();
            foreach (var t in await _db.GetXmlTagButtonsAsync())
                XmlTagButtons.Add(t);
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Could not load XML tag buttons."); }
    }
    ...
    // ── Populate FetchedFiles (flat list kept for backward compat) ─────────
    ...
    // ── Build file tree from fetched files (Phase 5) ───────────────────────

    private void BuildFileTree(List<(string Path, string Content)> files)
    {
        _treeRoots.Clear();
        var roots = FileTreeBuilder.Build(files);
        _treeRoots.AddRange(roots);

        // Sync exclusion state from FilePatternFilters
        foreach (var fileNode in FileTreeBuilder.AllFileNodes(_treeRoots))
        {
            fileNode.IsExcluded = FilePatternFilters.Any(f =>
                f.IsEnabled &&
                fileNode.FileName.Equals(f.Pattern, StringComparison.OrdinalIgnoreCase));
        }
        foreach (var folderNode in FileTreeBuilder.AllFolderNodes(_treeRoots))
            FileTreeBuilder.RecalculateFolderState(folderNode);
    ...
    private async Task FetchAsync()
    {
        ...
        _treeRoots.Clear(); _flatFileTree.Clear();
        ...
        try
        {
            ...
            {
                StatusText = "Scanning local folder…";
    ...
    // ── Auto-detect (FIX C-1: use injected IFileTypeDetectorService) ───────
    ...
    private async Task AutoDetectFileTypesAsync()
    {
        ...
        try
        {
            ...
            // FIX C-1: Use the injected FileTypeDetectorService instead of
            // the duplicated manual directory walk that was in this method.
            // For local paths this uses libgit2 (Windows/Mac) or directory scan.
            // For GitHub URLs this uses the lightweight Trees API (one HTTP request).
            var detectedExts = await _fileTypeDetector.DetectExtensionsAsync(
                inputPath,
            ...
            // Enable filters for detected extensions
            foreach (var f in FileTypeFilters)
                f.IsEnabled = f.Extensions.Any(e =>
                    detectedExts.Contains(e, StringComparer.OrdinalIgnoreCase));

            // Companion inference (e.g. .cs → enable .csproj)
            var inferredLabels = new List<string>();
            foreach (var detectedExt in detectedExts)
            {
            ...
            if (detectedExts.Any(e => e.Equals(".cs", StringComparison.OrdinalIgnoreCase)))
            {
            ...
            AutoDetectStatusText = $"✅ Detected {detectedExts.Count} type{(detectedExts.Count == 1 ? "" : "s")}{inferredPart}  ·  {enabledCount} filters enabled";
        }
        catch (Exception ex)
        {
    ...
    // ── File browser commands (legacy flat list) ───────────────────────────
    ...
    // ── Phase 5: File Tree commands ────────────────────────────────────────

    /// <summary>
    /// Handles a tap on a tree node:
    ///   - Folder node → toggle IsExpanded and rebuild the flat visible list.
    ///   - File node   → handled by ToggleTreeFileExclusionCommand for the icon tap.
    ///                   This command also toggles expansion if the file is tapped elsewhere.
    /// </summary>
    [RelayCommand]
    private void ToggleTreeNode(FileTreeNode node)
    {
    ...
    /// <summary>Tapping the ○/✕ icon on a file OR folder node toggles exclusion.</summary>
    [RelayCommand]
    private void ToggleTreeNodeExclusion(FileTreeNode node)
    {
        if (node is null) return;

        if (node.IsFolder)
        {
            // Exclude/include the whole folder and all descendants
            bool newState = !node.IsExcluded;
            node.IsExcluded          = newState;
            node.IsPartiallyExcluded = false;
            FileTreeBuilder.SetDescendantsExcluded(node, newState);
            FileTreeBuilder.UpdateAncestorState(node);

            // Sync with FolderFilters
            var folderName = node.Name.ToLowerInvariant();
            var existing = FolderFilters.FirstOrDefault(f =>
                f.Name.Equals(folderName, StringComparison.OrdinalIgnoreCase));
            if (newState && existing is null)
            {
                var ff = new FolderFilter { Name = node.Name, IsExcluded = true };
                ff.PropertyChanged += (_, _) => OnPropertyChanged(nameof(FolderFilterSummary));
                FolderFilters.Add(ff);
                OnPropertyChanged(nameof(FolderFilterSummary));
            }
            else if (!newState && existing is not null)
            {
                FolderFilters.Remove(existing);
                OnPropertyChanged(nameof(FolderFilterSummary));
            }
        }
        else
        {
            ...
            if (node.IsExcluded)
            {
                if (!FilePatternFilters.Any(f => f.Pattern.Equals(node.FileName, StringComparison.OrdinalIgnoreCase)))
                {
                    var pf = new FilePatternFilter { Pattern = node.FileName, IsEnabled = true, IsAutoAdded = true };
                    pf.PropertyChanged += (_, _) => OnPropertyChanged(nameof(FilePatternSummary));
                    FilePatternFilters.Add(pf);
                    OnPropertyChanged(nameof(FilePatternSummary));
                }
            ...
            else
            {
                var auto = FilePatternFilters.FirstOrDefault(f =>
                    f.IsAutoAdded &&
                    f.Pattern.Equals(node.FileName, StringComparison.OrdinalIgnoreCase));
                if (auto is not null)
                {
                    FilePatternFilters.Remove(auto);
                    OnPropertyChanged(nameof(FilePatternSummary));
                }
            }
        }
    ...
    {
        foreach (var folder in FileTreeBuilder.AllFolderNodes(_treeRoots))
    ...
    // ── Phase 4: XML Prompt Builder commands ──────────────────────────────

    /// <summary>Toggles the XML builder palette open/closed for the currently-editing prompt.</summary>
    [RelayCommand]
    private void ToggleBuilderMode(PromptItem prompt)
    {
        if (prompt is null) return;
        prompt.IsBuilderMode = !prompt.IsBuilderMode;
    }

    /// <summary>
    /// Appends an XML tag pair + placeholder to the currently-editing prompt's EditContent.
    /// For the special [PASTE CHUNK] button (empty XmlOpen), inserts the placeholder directly.
    /// </summary>
    [RelayCommand]
    private void AppendXmlTag(XmlTagButton tag)
    {
        if (tag is null) return;
        var editingPrompt = Prompts.FirstOrDefault(p => p.IsEditing);
        if (editingPrompt is null) return;

        editingPrompt.EditContent += tag.Insertion;
    }

    /// <summary>Raises an event so the code-behind can show a dialog to collect tag details.</summary>
    [RelayCommand]
    private void RequestAddCustomXmlTag() =>
        CustomXmlTagRequested?.Invoke(this, EventArgs.Empty);

    /// <summary>
    /// Called by code-behind after the user fills in the custom tag dialog.
    /// Creates and persists a new XmlTagButton.
    /// </summary>
    public async Task AddCustomXmlTagAsync(string label, string xmlOpen, string xmlClose, string placeholder)
    {
        if (string.IsNullOrWhiteSpace(label)) return;
        try
        {
            var tag = new XmlTagButton
            {
                Label       = label.Trim(),
                XmlOpen     = xmlOpen.Trim(),
                XmlClose    = xmlClose.Trim(),
                Placeholder = placeholder.Trim(),
                IsBuiltIn   = false,
                SortOrder   = XmlTagButtons.Count,
            };
            var saved = await _db.UpsertXmlTagButtonAsync(tag);
            tag.Id = saved.Id;
            XmlTagButtons.Add(tag);
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Could not save custom XML tag."); }
    }

    [RelayCommand]
    private async Task DeleteXmlTagButtonAsync(XmlTagButton tag)
    {
        if (tag is null || tag.IsBuiltIn) return;
        try
        {
            await _db.DeleteXmlTagButtonAsync(tag.Id);
            XmlTagButtons.Remove(tag);
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Could not delete XML tag button."); }
    }
    ...
    /// <summary>FIX F-9: Pastes clipboard text directly into the access token field.</summary>
    [RelayCommand]
    ...
    // ── Copy individual file content (chunk card code preview) ────────────

    /// <summary>
    /// Copies a single ChunkFile's raw content to clipboard.
    /// Triggered by the 📋 button in the code preview panel inside chunk cards.
    /// </summary>
    [RelayCommand]
    private async Task CopyFileContentAsync(ChunkFile file)
    {
        if (file is null) return;
        try
        {
            await _clipboard.SetTextAsync(file.Content);
            // Brief visual confirmation — reuse the IsCopied flash on the file row
            // by toggling IsCodeExpanded which triggers UI refresh on the parent chunk.
        }
        catch (Exception ex) { StatusText = $"⚠️ Could not copy: {ex.Message}"; }
    }
